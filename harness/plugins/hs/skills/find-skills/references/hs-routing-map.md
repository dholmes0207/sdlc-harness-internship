...
`hs:find-skills` uses this file for routing RULES + the high-traffic intent table below. This table is NOT the full catalog — the live catalog is `load_catalog()` (`harness/scripts/catalog.py`) plus `harness/scripts/disabled_skills.py --list`. ALWAYS merge those live sources before concluding a gap; a skill absent from this file may still exist.

**Selection rules**:

- Use the most specific skill first; router skills (`find-skills`, `triage`, `understand`) serve genuine orchestration only, not mere uncertainty.
