...
|---|---|
| `write-deny-policy.yaml` (soft tier) + `write_deny_policy.py` (hard floor, code) | The write-RBAC deny-list — the agent cage. Wrong edits silently widen what an agent can write. The hard floor is a code constant; only `soft_rules` is config. |
...
**Workflow `--fix` code-lane.** Under the deny-list write-RBAC a Workflow-orchestrated `--fix` runs as a `workflow-subagent` that edits project source out of the box — no lane setup, refused only the hard floor. The old overlay recipe is retired: `references/workflow-fix-overlay.md`.
