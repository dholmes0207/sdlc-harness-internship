...
End-to-end workflow for the **--auto** flag (brain-dump → spec hierarchy). A companion delta
**--update** flag was designed (single-artifact re-run of the decompose+confirm loop) but is
**not shipped** — absent from SKILL.md's Flags table; no `workflow-update.md` on disk. The
**No Silent Reversal** protocol below is real, shipped behavior running inline during `--auto`
itself; it does not depend on the missing `--update` flag.
