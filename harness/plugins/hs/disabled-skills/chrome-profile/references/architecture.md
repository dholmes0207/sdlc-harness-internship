...
Chrome launched normally on macOS/Linux/Windows starts one OS process per `user-data-dir`. All profiles inside that user-data-dir (`Default/`, `Profile 1/`, `Profile 17/`, etc.) share the same process; opening a "new profile window" from the UI does NOT spawn a new Chrome process — it opens a new top-level window owned by the same browser process, bound to a different internal `BrowserContext`.

Consequence: `chrome-devtools-mcp` attaching via CDP sees the **entire browser**, every profile window — targets across profiles appear in one flat list. CDP exposes `browserContextId` per target, but `chrome-devtools-mcp` currently does not surface it in `list_pages`. So from the MCP's outside-the-process view, profiles are invisible by default.
...
Without an anchor, after `chrome-profile cognition https://x` the agent only knows "a new tab in Cognition exists somewhere" — `list_pages` returns all tabs. Race conditions (e.g. user opens another tab manually in the same moment) make "find the newest tab" unreliable.
...
The profile marker is a sanity check/fallback; the open marker is primary because old tabs from prior runs can still carry the same profile marker.
...
Whatever the exact cause, the outcome is reproducible: cookies, saved passwords, and "stay signed in" state do not survive a copy. The only way to keep those is to keep using the original user-data-dir — staying inside the original Chrome process.
