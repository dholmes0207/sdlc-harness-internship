...
def _wire_target(target: Path, version_dir: Path, bin_root_value: str,
    ...
    # HARNESS_BIN_ROOT + HARNESS_DATA_ROOT per-project (F3) — never the global
    # ~/.claude. Wire the data root too (mirrors --global): the Bash-tool env
    # carries no CLAUDE_PROJECT_DIR, so without this the injected settings env
    # alone could not resolve state and a strict data_root() caller would fall
    # to the fail-closed marker whenever the SessionStart seed had not fired.
    _wire_env.wire_env(target, bin_root=bin_root_value,
                       data_root=str(target / ".harness"))
