...
DESIGN.md: agent-readable markdown file of extracted design tokens.
...
1. Paste `DESIGN.md` contents (or path) into the handoff instruction — no automatic detection exists
2. Implementation skill reads the tokens, uses them as constraints
3. If no DESIGN.md handed off: falls back to text-based design (existing behavior)
4. DESIGN.md > verbal design descriptions when explicitly handed off
...
2. Copy `DESIGN.md` to project root if skill expects it there
...
5. No modifications needed to existing skills — read DESIGN.md opportunistically
