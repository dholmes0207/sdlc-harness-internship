...
Views × formats matrix, the graph-JSON shape every renderer consumes, and the flag → view mapping. **SVG/PNG dropped** (validate gate decision) — no external Mermaid-CLI binary; HTML uses **Mermaid JS vendored inline** for offline self-containment.
