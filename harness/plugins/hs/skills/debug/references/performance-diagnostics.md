...
**Measure before optimizing.** Establish baseline + current state.

- What's expected response time vs actual?
...
These are optimizations to **recommend** in the debug report — hand them to `hs:fix` to apply; `hs:debug` identifies the bottleneck, it does not implement the change.
