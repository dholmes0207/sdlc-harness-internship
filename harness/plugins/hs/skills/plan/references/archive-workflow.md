...
- `phases/phase-*.md` - 20 first lines of each phase file to understand the progress and status

### Step 2: Summarize the plans and document them with `/hs:use journal` (the skill is install-disabled by default; `/hs:use` loads it)
Use `AskUserQuestion` tool to ask if user wants to document journal entries or not. Skip this step if user selects "No".
If user selects "Yes":
- Analyze the information in previous steps.

- Use Task tool with `subagent_type="hs:journal-writer"` in parallel to document all plans.
...
Use `AskUserQuestion` tool to ask if user wants to proceed with archiving these plans, select specific plans to archive or all completed plans only. Use `AskUserQuestion` tool to ask if user wants to delete permanently or move to the `./plans/archive` directory.
...
- Stage and commit the changes (Use `/hs:git` for commit flow)
- Commit and push the changes (Use `/hs:git` for push flow)
