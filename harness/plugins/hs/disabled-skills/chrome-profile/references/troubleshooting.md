...
1. **Chrome is not running.** The skill IPCs to a running Chrome holding the user-data-dir SingletonLock. If no Chrome is open, the second binary starts a fresh Chrome — same end result (tab opens in target profile), new window appears.
...
See SKILL.md's Agent Contract (the `doctor`-report / live-probe / `--force`-fallback rule) — same rule, not restated here.
...
The CLI opens the tab through Chrome's normal profile IPC, waits briefly, then reactivates the app frontmost before launch. On Linux and Windows this flag is currently a no-op.
