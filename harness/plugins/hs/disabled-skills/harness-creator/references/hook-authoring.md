...
The compliance wrapper handles: ImportError -> exit 2 + install command; crash -> exit 2 + audit trail + emergency bypass instructions. No external try/except needed.
...
3. **Registration required**: add the filename to `harness/install/hooks-registration.yaml`. CI test `TestComplianceRegistration` verifies this — unwired = no protection.
