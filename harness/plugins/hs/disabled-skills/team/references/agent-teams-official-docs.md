...
Agent Teams coordinate multiple Claude Code instances. One session acts as the team lead, coordinating work, assigning tasks, and synthesizing results. Teammates work independently, each in its own context window, and communicate directly with each other.
...
|---|---|---|
| **Tool** | `Agent` (formerly `Task`), no `name` | `Agent(name=…)` + `TaskCreate`/`SendMessage` (implicit team; no `TeamCreate`) |
...
| **Model** | Any (haiku/sonnet/opus per agent) | Per-teammate `model:` supported (haiku/sonnet/opus) |
...
Both require confirmation — Claude won't create a team without approval.
...
Task dependencies managed automatically -- completing a blocker unblocks dependents.

## Tools API Surface

### Agent Tool (spawn teammates)

The `Agent` tool (formerly `Task`, renamed v2.1.63) spawns teammates. `name` promotes a spawn from one-shot subagent to persistent teammate — present in the tool schema only when the gate is on. Omit `name` → plain subagent.

```
Agent(
  name: string,                # REQUIRED for a teammate (gate-only param); omit → plain subagent
  ...
  model: "sonnet",             # Optional per-teammate model (haiku/sonnet/opus)
  run_in_background: true,     # Non-blocking spawn
  isolation: "worktree"        # Optional: git worktree isolation
)
```

**Built-in subagent types:** `general-purpose`, `Explore`, `Plan`, `hs:researcher`, `hs:developer`, `hs:code-reviewer`, `hs:debugger`, `hs:tester`, `hs:planner`, `hs:docs-manager`, `hs:brainstormer`, and more.

**Custom subagents:** Define in `.claude/agents/` with frontmatter (name, description, tools, model).

### Team creation / teardown (implicit — no tools)

`TeamCreate` and `TeamDelete` were **removed in CC v2.1.178** and are filtered out of the tool surface. The team is implicit: **one per session, session-derived name**, established by the first named `Agent(...)` spawn. No teardown call — after teammates approve `shutdown_request`, ending the session releases the team. Attempting a second team returns the server error "one team per
session".
...
Task claiming uses file locking to prevent race conditions. Task dependencies resolve automatically -- completing a blocker unblocks dependents.

## Hook Events

These are Claude Code platform events. the harness does not ship custom handlers for them by default; team workflows should monitor TaskList and teammate messages unless a project adds and validates its own handlers.
...
Scales with active teammates. Worth it for research/review/features. Single session more cost-effective for routine tasks. Mixed-model teams are supported -- set `model:` per teammate to match task weight.
