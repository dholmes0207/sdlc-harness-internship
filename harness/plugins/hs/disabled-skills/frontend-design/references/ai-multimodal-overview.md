...
Entry point for `hs:ai-multimodal` — generate/analyze visual assets aligned with frontend design thinking and aesthetic guidelines.

## When to Use

Use `hs:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `hs:frontend-design` with `hs:ai-multimodal` for asset generation and analysis that follows those guidelines.
