...
_RBAC_SCOPE = (
    ...
    "name and its hs:-prefixed form resolve) — to see your write scope. Under the "
    "deny-list model you may write ANYWHERE except the hard floor (the protected core "
    "+ the harness binary). A write into the floor is BLOCKED by agent_rbac_guard / "
    "floor_bash_guard — if a task needs it, STOP and return the raw output/result "
    "instead. You cannot widen the floor yourself (it is caged); a soft-tier widen is a "
    "human decision via deny_audit.py --request-widen."
)
