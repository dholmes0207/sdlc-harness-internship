...
Stitch SDK has no programmatic quota check endpoint. SDLC Harness tracks locally:

**File:** `~/.sdlc-harness/.stitch-quota.json`
...
2. Skill prints: "Daily quota exhausted. Use hs:ui-ux as fallback."
3. Activate `hs:ui-ux` with the same design prompt
4. `ui-ux` generates text-based design spec (no external API needed)
5. Proceed with implementation using text-based spec

## Drift Warning

Local tracking can drift if user generates designs outside SDLC Harness (Stitch web UI or other tools). If `RATE_LIMITED` hits despite local tracker showing credits available:
