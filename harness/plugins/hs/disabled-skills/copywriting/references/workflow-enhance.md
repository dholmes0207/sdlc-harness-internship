...
Analyze copy issues, enhance for better performance.
...
1. **Screenshots provided** → Use `hs:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `hs:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/hs:scout ext` (preferred) or `/hs:scout` to find files needing enhancement
4. **Implement** → Use `hs:developer` agent to write enhanced copy into code files
