...
The journal records that a decision was made and why, as **history** — it is **not** the decision's home. A durable decision belongs in the DEC ledger (`docs/decisions.md` via `hs:remember` / `decision_register.py`), not the journal entry.
...
Create journal entries in `./plans/journals/` named `YYYY-MM-DD-<slug>.md`.
