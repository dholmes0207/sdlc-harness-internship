...
- **concept**: Subject/description (required)
...
Use `AskUserQuestion` with 4 questions in one call:
...
**If "Edit prompt":** Ask for edited text; use instead.
**If "Start over":** Return to Step 2.

Run generation:
```bash
cd "${HARNESS_BIN_ROOT:-.}"/harness/plugins/hs/skills/ai-artist && python3 scripts/generate.py "[concept]" \
