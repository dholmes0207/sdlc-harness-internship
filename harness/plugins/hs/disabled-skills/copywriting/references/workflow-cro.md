...
Analyze content, optimize conversion from user-reported issues.
...
1. **Screenshots provided** → Use `hs:ai-multimodal` skill to analyze conversion issues
2. **Videos provided** → Use `hs:ai-multimodal` video-analysis for bottleneck identification
3. **URL provided** → Use `web_fetch` tool to fetch and analyze current issues
4. **Scout codebase** → `/hs:scout ext` (preferred) or `/hs:scout` to find relevant files
5. **Implement** → Use `hs:developer` agent to write enhanced copy into code files

## CRO Plan Creation Workflow

Use when creating a structured CRO optimization plan (e.g., via `/hs:plan --hard` with CRO-specific requirements).

### Steps

1. **Analyze** → Gather issues from screenshots/videos/URLs using steps above
2. **Activate** `hs:plan` skill for plan structure
...
6. **Create phase files** (`phases/phase-N-name.md`) with sections: Context links, Overview (date/priority/statuses), Key Insights, Requirements, Architecture, Related code files, Implementation Steps, Todo list, Success Criteria, Risk Assessment, Security Considerations, Next steps
