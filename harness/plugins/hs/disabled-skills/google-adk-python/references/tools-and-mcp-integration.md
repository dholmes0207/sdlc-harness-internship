...
Verify installed `google-adk` version: recent releases preserve agent session on MCP tool error. If a project drops session after tool failure, upgrade `google-adk` before adding workaround state recovery.
